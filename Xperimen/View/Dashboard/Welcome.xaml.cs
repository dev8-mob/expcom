...
namespace Xperimen.View.Dashboard
{
    ...
    public partial class Welcome : ContentPage
    {
        ...
        public async void MoreInfoClicked(object sender, EventArgs e)
        {
            ...
            view.IsEnabled = false;
            viewmodel.IsLoading = true;
            var build = "Income : RM " + string.Format("{0:0.00}", viewmodel.Income) + "\r\nTotal Commitment : RM " + string.Format("{0:0.00}", viewmodel.TotalCommitment)
                + "\r\nTotal Expenses : RM " + string.Format("{0:0.00}", viewmodel.TotalExpenses);
            var avail = "Available : RM " + string.Format("{0:0.00}", viewmodel.BalanceAvailable);
